...
import { AmplifyClient, CreateAppCommand, CreateAppCommandInput, ListAppsCommand, Platform } from '@aws-sdk/client-amplify'
import { STSClient, GetCallerIdentityCommand } from "@aws-sdk/client-sts";
import * as yaml from "js-yaml";
import { Config, validateConfig, AmplifyAppsResponse } from "../types.js";
import fs from "fs";
...
export const getConfig = async (): Promise<Config> => {
    ...
    // convert to object
    const configObject: unknown = yaml.load(configYaml);
    const config: Config = validateConfig(configObject);

    config.githubRepo = config.githubRepo
        .replace(/^https:\/\/github.com\//, "")
        .replace(/\.git$/, "");

    const confirmedConfig = await confirmAction(
...
export const gitActions = {
    promptForGitHubToken: (): Promise<string> => {
        const rl = readline.createInterface({
            input: process.stdin,
            output: process.stdout,
        });
            ...
            rl.question(
                ...
                (token) => {
                    ...
                    if (!tokenRegex.test(token) && !tokenRegex2.test(token)) {
                        console.error(
                            `Invalid GitHub Personal Access Token format. ${tokenRegex}`
                        );
                        process.exit(1);
                    }
                    resolve(token);
    ...
    checkRepoAccess: async (repo: string): Promise<string> => {
        ...
        if (permissions.admin || permissions.push) {
            console.log(
                `You have read/write access to the repository: ${repo}`
            );
            return token;
        }
        throw new Error(
            `You do not have write access to the repository: ${repo}`
        );
    },
    checkAppRepoHasNoBranches: async (config: Config, token: string) => {
        const response = await fetch(
            `https://api.github.com/repos/${config.githubRepo}/branches`,
            {
                headers: {
                    Authorization: `token ${token}`,
                },
        ...
        if (!response.ok) {
            throw new Error(
                `Failed to fetch repository branches: ${response.statusText}`
            );
        }
        const branches = await response.json();
        if (branches.length > 0) {
            throw new Error(
                `Repository ${config.githubRepo} already has branches please use a new repository`
            );
        }
    },
    createGitHubRepo: async (repoPath: string, token: string) => {
        ...
        try {
            ...
            await execa(
                "git",
                ["commit", "-m", "first commit from bootstrap"],
                {
                    stdio: "inherit",
                }
            );
            ...
            if (error instanceof Error) {
                ...
                console.error(
                    `Failed to create GitHub repository: ${String(error)}`
                );
            }
        }
    },
    pushDirectoryToGithub: async (repoPath: string, token: string)=>{
...
export const awsActions = {
    getTags: (config: Config): { Key: string; Value: string }[] => {
        const awsTags = [
            {
    ...
    checkProfileAccess: async (config: Config) => {
        ...
        console.log(
            chalk.blue(actionNames.map((action) => ` - ${action}`).join("\n"))
        );
        ...
        const notAllowed = res.EvaluationResults?.some(
            (o) => o.EvalDecision !== "allowed"
        );
        if (notAllowed) {
            throw new Error("Insufficient permissions for role: " + baseArn);
        }
    },
    checkAndCreateRole: async (config: Config): Promise<string> => {
        process.env.AWS_PROFILE = config.awsProfile;
        ...
        try {
            ...
            if (e instanceof Error && e.name === "NoSuchEntityException") {
                if (!config.amplifyRole.createRole) {
                    throw new Error(
                        "Role does not exist and createRole is false"
                    );
                }
                const commandProps = {
                    RoleName: config.amplifyRole.roleName,
                ...
                const confirmedRole = await confirmAction(
                    "Please confirm role creation: \n" +
                ...
                console.log(
                    chalk.blue(
                        `Creating role ${
                            config.amplifyRole.roleName
                        } for Amplify with config:\n ${JSON.stringify(
                            commandProps,
                            null,
                            2
                        )}`
                ...
                const res = await client.send(
                    new CreateRoleCommand(commandProps)
                );
                roleArn = res.Role?.Arn;
                if(!roleArn) {
        ...
        const isPolicyAttached = response.AttachedPolicies?.some(
        ...
        if (!isPolicyAttached) {
            ...
            const confirmRoleAttachment = await confirmAction(
                "Please confirm policy attachment to role: \n" +
                    JSON.stringify(commandProps, null, 2)
            );
            ...
            console.log(
                chalk.blue(
                    `Attaching policy ${policyArn} to role ${config.amplifyRole.roleName}`
                )
            );
            await client.send(new AttachRolePolicyCommand(commandProps));
            console.log(
                chalk.green(
                    `Policy ${policyArn} attached to role ${config.amplifyRole.roleName}`
                )
            );
        }
        console.log(
            chalk.green(
                `Role ${config.amplifyRole.roleName} created successfully`
            )
        );
        return roleArn;
    },
    createApp: async (config: Config, token: string, amplifyRoleArn: string) => {
        process.env.AWS_PROFILE = config.awsProfile;
        const existingAppId = await awsActions.getAmplifyAppId(config);
        if (existingAppId) {
            throw Error(
                `Amplify project already exists: ${JSON.stringify({
        ...
        const commandProps:CreateAppCommandInput = {
            name: config.appName,
    ...
    getAmplifyAppId: async (config: Config): Promise<string | undefined> => {
        process.env.AWS_PROFILE = config.awsProfile;
        const client = new AmplifyClient({ region: config.awsRegion });
