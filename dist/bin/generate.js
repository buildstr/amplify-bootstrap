...
async function generateApp() {
    ...
    try {
        ...
        if (!fs.existsSync('./application')) {
            ...
            try {
                ...
                for (const file of appFiles) {
                    ...
                    if (file.type === 'file') {
                        fs.copyFileSync(file.content, path.join(appDir, file.name));
                        continue;
                    }
                    fs.writeFileSync(path.join(appDir, file.name), file.content);
                }
            ...
            console.log(chalk.yellow('Next.js application dir already exists. to re-generate, delete the application directory and run the App Generation script again.'));
        }
        ...
        if (error instanceof Error) {
            ...
            console.error(chalk.red('App Generation failed:', error));
        }
        process.exit(1);
    }
}
